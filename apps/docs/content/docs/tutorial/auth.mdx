---
title: "Auth"
description: "Add authentication and CASL-based authorization with sessions, OAuth/OIDC, middleware, and one unified guard."
---

The `@nest-boot/auth` module provides authentication powered by [better-auth](https://www.better-auth.com/) and CASL-based authorization. It includes MikroORM session persistence, automatic middleware registration, and one `AuthGuard` for both authentication and route permission checks.
...
The same globally registered `AuthGuard` first enforces authentication and then evaluates `@Can` metadata. No second permission guard or module is required. `@Public()` only skips the session requirement; if a public route also declares `@Can`, its permission is still checked.
