...
4. **RLS Context** — When combining auth with PostgreSQL RLS, load the tenant or workspace in `onAuthenticated` and write it to `RowLevelSecurityContext`. See the [Row-Level Security tutorial](/docs/tutorial/row-level-security).
