...
When no options are passed, the module auto-loads configuration with `loadConfigFromEnv()`. It reads `DATABASE_URL` and parses the URL into structured MikroORM options instead of passing the URL to a database client. The supported schemes are `postgresql:` and its `postgres:` alias for PostgreSQL, `mysql:` for MySQL, and `file:` for SQLite. Use an absolute file URL such as `file:///var/lib/app/app.db` for SQLite. Pass explicit module options for non-URL configuration or debug logging.
