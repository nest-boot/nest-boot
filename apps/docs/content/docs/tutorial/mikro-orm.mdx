...
When no options are passed, the module auto-loads configuration with `loadConfigFromEnv()`. It reads `DB_URL`, falling back to `DATABASE_URL`, and parses the URL into driver, host, port, database name, user, password, schema, and structured driver query options. The `postgres`/`postgresql` and `mysql` protocols select their corresponding drivers. Other individual `DB_*` and `DATABASE_*` variables are not read; pass explicit module options for non-URL configuration or debug logging.
