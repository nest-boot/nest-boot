...
When no options are passed, the module auto-loads configuration with `loadConfigFromEnv()`. It reads `DATABASE_URL` and parses the URL into driver, host, port, database name, user, password, schema, and structured driver query options. The `postgres`/`postgresql` and `mysql` protocols select their corresponding drivers. Pass explicit module options for non-URL configuration or debug logging.
