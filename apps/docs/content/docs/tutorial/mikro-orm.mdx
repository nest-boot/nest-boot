...
When no options are passed, the module auto-loads configuration with `loadConfigFromEnv()`. `DB_URL` or `DATABASE_URL` takes precedence; otherwise use the `DB_*` or `DATABASE_*` host, port, database, user, password, and type variables. URL protocols and `DB_TYPE`/`DATABASE_TYPE` currently resolve `postgres`/`postgresql` and `mysql` drivers.
