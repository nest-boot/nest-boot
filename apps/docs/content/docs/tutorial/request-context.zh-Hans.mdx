...
显式写入上下文的值优先级最高。当类或 Token 不在上下文中时，
`RequestContextModule` 创建的 HTTP 上下文会从 Nest 应用容器中延迟获取单例
Provider。这适合在请求内执行、但创建位置不处于 Nest 依赖注入中的回调：
...
同步回退不会解析请求作用域 Provider；此类实例应通过
`RequestContext.set()` 显式写入。
...
`RequestContext.current().type` 是创建上下文时指定的应用级生命周期分类，并不等同于 NestJS 的 `ExecutionContext.getType()`。

请求中间件或兜底拦截器自动创建的上下文统一使用 `"http"`。GraphQL 解析器也遵循这一规则：NestJS 会将解析器的执行上下文类型报告为 `"graphql"`，而 `RequestContext.type` 仍为 `"http"`，从而保证中间件路径和兜底路径对同一类请求采用一致的分类。

其他集成可以根据所创建的生命周期选择不同分类。例如，内置 BullMQ 处理器封装使用 `"queue"`，REPL 使用 `"repl"`。直接创建上下文时，应选择描述其生命周期的分类，例如 `"job"`。如果应用需要区分 GraphQL 执行类型、请求上下文分类或底层传输，应将其作为独立的上下文元数据存储，而不是依赖 `type` 同时表达这些概念。
