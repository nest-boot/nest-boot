...
模块自动读取 `REDIS_URL`。没有 URL 时，它使用 `REDIS_HOST`、`REDIS_PORT`、`REDIS_DB`/`REDIS_DATABASE`、`REDIS_USER`/`REDIS_USERNAME`、`REDIS_PASS`/`REDIS_PASSWORD` 和 `REDIS_TLS`。也可以通过 `BullModule.forRoot()` 或 `forRootAsync()` 显式传入 BullMQ `connection`。
...
Nest Boot 的 `@Processor()` 会为每个任务创建 `RequestContext`、写入 BullMQ `JOB_REF`，再调用正常的 `@nestjs/bullmq` processor。
...
`Job` 必须是继承导出基类 `JobEntity` 的具体 MikroORM 实体，并被实体发现配置纳入。集成会发现已注册的 queue 和 worker、upsert 状态变化，并通过每小时任务删除超过 `jobTTL` 的历史记录（默认 30 天）。可通过 `includeQueues` 和 `excludeQueues` 控制持久化范围；`convertJobToEntityData` 用于映射额外的实体字段。
