...
Values stored explicitly in the context take precedence. When a class or token
is not present, HTTP contexts created by `RequestContextModule` lazily resolve
singleton providers from the Nest application container. This is useful in
callbacks that run inside a request but are created outside Nest dependency
injection:
...
Request-scoped providers are not resolved by this synchronous fallback. Store
those explicitly with `RequestContext.set()`.
...
`RequestContext.current().type` is an application-level lifecycle category selected when the context is created. It does not mirror NestJS `ExecutionContext.getType()`.

Contexts created automatically by the request middleware or fallback interceptor use `"http"`. This includes GraphQL resolver executions: NestJS reports `"graphql"` for the resolver execution context, while `RequestContext.type` remains `"http"` so middleware and fallback paths classify the same request consistently.

Other integrations can select categories for the lifecycle they create. For example, the built-in BullMQ processor wrapper uses `"queue"`, and the REPL uses `"repl"`. For contexts you create directly, choose a descriptive category such as `"job"`. If an application needs to distinguish GraphQL execution from its request context category or transport, store that as separate context metadata instead of relying on `type`.
