...
For contexts you create manually, choose a category that describes the lifecycle, such as `"job"` or `"repl"`. If an application needs to distinguish GraphQL execution from its request context category or transport, store that as separate context metadata instead of relying on `type`.
