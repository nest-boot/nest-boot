...
- Apollo's local landing page unless `playground` is explicitly `false`
...
- When `REDIS_URL` or `REDIS_HOST` is set, it uses the distributed Redis driver. `REDIS_PORT`, database, username, password, and TLS variables supplement `REDIS_HOST`; `REDIS_URL` takes precedence over all individual settings.
- When neither `REDIS_URL` nor `REDIS_HOST` is set, it uses the process-local memory driver.
