...
- When any supported Redis variable is set (`REDIS_URL`, `REDIS_HOST`, `REDIS_PORT`, `REDIS_DB` / `REDIS_DATABASE`, `REDIS_USER` / `REDIS_USERNAME`, `REDIS_PASS` / `REDIS_PASSWORD`, or `REDIS_TLS`), it uses the distributed Redis driver. `REDIS_URL` takes precedence over the individual settings.
- When none of those variables are set, it uses the process-local memory driver.
...
Custom drivers extend `GraphQLRateLimitDriver` and implement its atomic `update()` method. Positive `points` consume capacity and negative `points` restore it. An explicitly configured driver takes precedence over environment-driven Redis selection; `forRootAsync()` can be used when creating it requires injected dependencies.
