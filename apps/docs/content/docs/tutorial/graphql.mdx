...
- When `REDIS_URL` is set, it uses the distributed Redis driver.
- When `REDIS_URL` is absent, it uses the process-local memory driver.
...
Custom drivers extend `GraphQLRateLimitDriver` and implement its atomic `update()` method. Positive `points` consume capacity and negative `points` restore it. An explicitly configured driver takes precedence over `REDIS_URL`; `forRootAsync()` can be used when creating it requires injected dependencies.
