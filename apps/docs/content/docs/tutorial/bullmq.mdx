...
The module reads `REDIS_URL` automatically. Without a URL it uses `REDIS_HOST`, `REDIS_PORT`, `REDIS_DB`/`REDIS_DATABASE`, `REDIS_USER`/`REDIS_USERNAME`, `REDIS_PASS`/`REDIS_PASSWORD`, and `REDIS_TLS`. You can instead call `BullModule.forRoot()` or `forRootAsync()` with an explicit BullMQ `connection`.
