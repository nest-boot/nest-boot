...
- 存在 `REDIS_URL` 时，使用分布式 Redis 驱动。
- 不存在 `REDIS_URL` 时，使用进程内 memory 驱动。
...
自定义驱动需要继承 `GraphQLRateLimitDriver`，并实现具备原子性的 `update()` 方法。正数 `points` 消耗容量，负数则恢复容量。显式配置的驱动优先于 `REDIS_URL`；如果创建驱动需要注入其他依赖，可以使用 `forRootAsync()`。
