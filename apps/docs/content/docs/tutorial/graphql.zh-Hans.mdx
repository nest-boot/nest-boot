...
- 配置任意受支持的 Redis 环境变量（`REDIS_URL`、`REDIS_HOST`、`REDIS_PORT`、`REDIS_DB` / `REDIS_DATABASE`、`REDIS_USER` / `REDIS_USERNAME`、`REDIS_PASS` / `REDIS_PASSWORD` 或 `REDIS_TLS`）时，使用分布式 Redis 驱动；其中 `REDIS_URL` 优先于各项独立配置。
- 上述环境变量均未配置时，使用进程内 memory 驱动。
...
自定义驱动需要继承 `GraphQLRateLimitDriver`，并实现具备原子性的 `update()` 方法。正数 `points` 消耗容量，负数则恢复容量。显式配置的驱动优先于根据环境变量选择的 Redis 驱动；如果创建驱动需要注入其他依赖，可以使用 `forRootAsync()`。
