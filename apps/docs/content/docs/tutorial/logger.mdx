...
Pass an `Error` directly or in structured bindings (`{ err: error }`) to retain its message and stack under `err`. Explicit stack strings are emitted as `stack`. A single plain string after the message remains a context override; a stack-looking string retains the logger's current context.

```typescript
this.logger.error(error);
this.logger.error("Operation failed", error.stack, "OrdersService");
this.logger.error("Operation failed", { err: error, orderId });
```
