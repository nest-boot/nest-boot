...
不传参数时，模块使用 `loadConfigFromEnv()` 自动加载配置。它读取 `DATABASE_URL`，并把 URL 解析为 driver、host、port、数据库名、用户名、密码、schema 和结构化驱动查询选项。`postgres`/`postgresql` 与 `mysql` 协议会选择对应驱动。非 URL 配置或调试日志请显式传入模块选项。
