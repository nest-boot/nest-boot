...
不传参数时，模块使用 `loadConfigFromEnv()` 自动加载配置。`DB_URL` 或 `DATABASE_URL` 优先；否则使用 `DB_*` 或 `DATABASE_*` 的 host、port、database、user、password 和 type 变量。URL protocol 与 `DB_TYPE`/`DATABASE_TYPE` 当前可解析 `postgres`/`postgresql` 和 `mysql` 驱动。
