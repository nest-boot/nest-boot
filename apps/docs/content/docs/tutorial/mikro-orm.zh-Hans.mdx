...
不传参数时，模块使用 `loadConfigFromEnv()` 自动加载配置。它优先读取 `DB_URL`，否则读取 `DATABASE_URL`，并把 URL 解析为 driver、host、port、数据库名、用户名和密码选项。`postgres`/`postgresql` 与 `mysql` 协议会选择对应驱动。模块不会读取其他独立的 `DB_*` 和 `DATABASE_*` 变量；非 URL 配置或调试日志请显式传入模块选项。
