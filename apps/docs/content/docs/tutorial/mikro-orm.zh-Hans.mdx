...
不传参数时，模块使用 `loadConfigFromEnv()` 自动加载配置。它读取 `DATABASE_URL`，并解析为结构化 MikroORM 选项，而不会把 URL 交给数据库客户端解析。支持的 scheme 是 PostgreSQL 的 `postgresql:` 及其 `postgres:` 别名、MySQL 的 `mysql:`，以及 SQLite 的 `file:`。SQLite 请使用 `file:///var/lib/app/app.db` 这样的绝对文件 URL。非 URL 配置或调试日志请显式传入模块选项。
