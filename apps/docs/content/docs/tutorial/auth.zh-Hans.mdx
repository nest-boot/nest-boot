---
title: "Auth"
description: "通过会话、OAuth/OIDC、中间件与统一守卫提供身份认证和基于 CASL 的授权。"
---

`@nest-boot/auth` 模块提供基于 [better-auth](https://www.better-auth.com/) 的身份认证与基于 CASL 的授权，包括 MikroORM 会话持久化、自动中间件注册，以及同时负责认证和路由权限检查的统一 `AuthGuard`。
...
同一个全局 `AuthGuard` 会先检查身份认证，再执行 `@Can` 权限元数据，不需要注册第二个权限模块或守卫。`@Public()` 只跳过会话要求；如果公开路由同时声明了 `@Can`，权限检查仍会执行。
