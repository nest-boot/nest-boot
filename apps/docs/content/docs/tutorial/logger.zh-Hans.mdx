...
| `error(message, trace?, context?)` | ERROR 级别日志，可选堆栈跟踪                   |
| `debug(message)`                   | DEBUG 级别日志                                 |

直接传入 `Error`，或通过结构化参数 `{ err: error }` 传入，可在 `err` 中保留错误消息与堆栈。显式传入的堆栈字符串写入 `stack`。消息后的单个普通字符串仍表示 context；符合堆栈格式的字符串则保留 Logger 当前的 context。
