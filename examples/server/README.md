...
2. `Migration20260917173928`: generated by MikroORM without a custom name, including tables, indexes, foreign keys with deletion rules, and RLS policies. It creates separate user/workspace API-key tables. Membership is unique by `(user, workspace)`; contact emails may be shared. No handwritten grants or function definitions are mixed in.
