...
roles, entity tables, foreign-key cascades, ownership CHECK constraints, RLS policies
and uniform default privileges on an empty database. User and Session policies read the
trusted `app.user.permissions` JSON array; no custom SQL functions or auth schema
are needed. Services retain separate authorization checks.
API keys have exactly one of `user` / `workspace` set. Deleting a user cascades to
...
2. `Migration20260915035801`: generated by MikroORM without a custom name, including tables, indexes, foreign keys with deletion rules, ownership CHECK constraints, and RLS policies. No handwritten grants or function definitions are mixed in.
3. `Migration20260915073637`: generated by MikroORM to remove the member contact-email unique constraint. Membership remains unique by `(user, workspace)`; contact emails may be shared. Resolve any duplicate contact emails before rolling this migration back.
