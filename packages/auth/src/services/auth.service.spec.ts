...
describe("current user identity", () => {
  ...
  it("returns application users and adopts only issued sessions", async () => {
    ...
    authMiddleware.authenticateSession.mockResolvedValue(user);
    expect(
      (
        await service.signInEntity({
          email: "new@example.com",
          password: "password",
        })
      ).user,
    ...
    expect(
      (await service.signInSocialEntity({ provider: "test" })).user,
    ).toBeNull();
    expect(authMiddleware.authenticateSession).toHaveBeenCalledOnce();
  });
  it("requires and returns the request's user identity in AuthService", async () => {
...
describe("AuthService", () => {
  ...
  it("verifies the authenticated user's credential password", async () => {
    ...
    await expect(service.verifyCurrentUserPassword("password")).resolves.toBe(
      true,
    );
  ...
  it("updates the current user without exposing the Better Auth API", async () => {
    ...
    await expect(service.updateCurrentUser(options)).resolves.toBe(true);
  ...
  it("applies refreshed session cookies when updating the current user", async () => {
    ...
    const { appendHeader, result } = await withResponse(() =>
      service.updateCurrentUser(options),
    );
  ...
  it("changes the current user's email", async () => {
    ...
    await expect(service.changeCurrentUserEmail(options)).resolves.toBe(true);
  ...
  it("applies refreshed session cookies during an email change", async () => {
    ...
    const { appendHeader, result } = await withResponse(() =>
      service.changeCurrentUserEmail(options),
    );
    expect(result).toBe(true);
    expect(appendHeader).toHaveBeenCalledWith(
      "Set-Cookie",
  ...
  it("changes the password and only exposes the replacement token", async () => {
    ...
    await expect(service.changeCurrentUserPassword(options)).resolves.toEqual({
      token: "replacement-token",
    });
  ...
  it("applies the replacement session cookie after a password change", async () => {
    ...
    const { appendHeader, result } = await withResponse(() =>
      service.changeCurrentUserPassword(options),
    );
  ...
  it("adds a password to an account without credentials", async () => {
    ...
    await expect(service.setCurrentUserPassword("new-password")).resolves.toBe(
      true,
    );
  ...
  it("requests user deletion with empty default options", async () => {
    ...
    await expect(service.deleteCurrentUser()).resolves.toEqual({
      message: "User deleted",
      success: true,
    });
  ...
  it("lists safe linked-account summaries", async () => {
    ...
    await expect(service.listCurrentUserAccounts()).resolves.toBe(accounts);
  ...
  it("starts a social account-linking flow", async () => {
    ...
    await expect(service.linkCurrentUserAccount(options)).resolves.toBe(result);
  ...
  it("applies account-linking state cookies", async () => {
    ...
    const { appendHeader, result } = await withResponse(() =>
      service.linkCurrentUserAccount(options),
    );
    expect(result).toEqual(response);
    expect(appendHeader).toHaveBeenCalledWith(
      "Set-Cookie",
  ...
  it("unlinks an account", async () => {
    ...
    await expect(service.unlinkCurrentUserAccount(options)).resolves.toBe(true);
  ...
  it("returns provider account information", async () => {
    ...
    await expect(
      service.getAccountInfo({ accountId: "account-1" }),
