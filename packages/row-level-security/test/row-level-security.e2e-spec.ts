...
@Policy({
  name: POLICY_MIGRATION_POLICY_NAME,
  command: PolicyCommand.SELECT,
  using: `(select current_setting('app.workspace_id'::text, true)::integer) = workspace_id`,
})
@Entity({
  schema: POLICY_MIGRATION_SCHEMA_NAME,
  tableName: POLICY_MIGRATION_TABLE_NAME,
...
describeIfDatabaseConfigured("RowLevelSecurity - database integration", () => {
  ...
  it("reads transaction-local context settings with missing-ok casts", async () => {
    const row = await withTransaction(async (trx) => {
      await execute("select set_config('app.user_id', '42', true)", trx, "run");

      return await execute<{ user_id: number; missing_id: number | null }>(
        "select current_setting('app.user_id', true)::integer as user_id, current_setting('app.missing_id', true)::integer as missing_id",
        trx,
        "get",
      );
    });
  ...
  it("enforces generated row level security policies with app context", async () => {
    ...
    const rows = await withTransaction(async (trx) => {
      await execute("set local role authenticated", trx, "run");
      await execute(
      ...
      return await execute<
        { id: number; tenant_id: number; content: string }[]
      >(
      ...
      async () => {
        ...
        return await orm.em.transactional(async (em) => {
          ...
          return await em.getConnection().execute<{
            current_user: string;
            tenant_id: number;
          }>("select current_user, current_setting('app.tenant_id', true)::integer as tenant_id", [], "get", trx);
        });
      },
    );
  ...
  it("clears row level security state when disabled inside an existing transaction", async () => {
    const result = await RequestContext.run(
      ...
      async () => {
        ...
        return await orm.em.transactional(async (em) => {
          ...
          }>("select current_user, current_setting('app.tenant_id', true) as tenant_id", [], "get", trx);
    ...
    expect(result.disabled.tenant_id).toBe("");
  });
  ...
  it("clears row level security state without RequestContext inside an existing transaction", async () => {
    ...
    expect(result.unscoped.tenant_id).toBe("");
  });
  ...
  it("generates a policy diff after applying a migration and changing only context", async () => {
    ...
    try {
      ...
      ).create(migrationPath, false, false, "UpdatePolicyContext");
  ...
  async function expectFieldChangeMigrationToKeepPolicy({
    ...
    try {
      ...
      ).createInitial(migrationPath, initialMigrationName);
      ...
      ).create(migrationPath, false, false, fieldChangeMigrationName);
  ...
  async function resetSchema() {
    await dropSchema();
    await orm.schema.create({ schema: DOCUMENT_SCHEMA_NAME });
  }
  ...
  async function createPolicyFixture() {
    ...
    await runStatements(
      createPolicyUpSqlStatements({
        ...
        roles: ["authenticated"],
        using:
          "((select current_setting('app.tenant_id', true)::integer) = tenant_id)",
        withCheck:
          "((select current_setting('app.tenant_id', true)::integer) = tenant_id)",
      }),
    );
  ...
  async function withTransaction<T>(
    callback: (trx: Transaction) => T | Promise<T>,
  ) {
    return await orm.em.transactional(async (em) => {
      const trx = em.getTransactionContext();
